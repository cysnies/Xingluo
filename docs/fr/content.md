...
- Les URL des articles sont dérivées du chemin du fichier (voir la section routage dans [Aperçu de l'architecture](./architecture.md))
...
| `modDatetime`    | date                     | —               | Date de mise à jour ; affichée avec la date de publication                                                                                                                                       |
...
La couche de routage résout automatiquement la traduction correcte par langue et déduplique dans les listes — le même article dans différentes langues n'affiche qu'une carte par langue. Les articles sans traduction reviennent au contenu original. Voir [Internationalisation](./i18n.md).
...
(La table des matières est remplie automatiquement ici)
```
...
Voir [Lecteurs multimédia](./media-players.md) pour plus de détails.

### Désactivation de MDX
...
Le système de commentaires est rendu automatiquement en bas des pages de détail des articles (configurez le fournisseur dans `features.comments`). Voir [Système de commentaires](./comments.md).
