...
2. **`astro build`**：靜態生成全站到 `dist/`（含動態 OG 圖、RSS、sitemap、robots.txt、Flexsearch 搜索索引）
3. **`node scripts/generateSearchIndex.mjs`**：掃描 `dist/` 生成 Flexsearch 搜索索引到 `dist/search/`

> 註：搜索索引生成腳本基於 Flexsearch，構建時自動執行。
...
- 閱讀 [設定指南](./configuration.md) 定製站點資訊與功能開關
- 閱讀 [內容創作](./content.md) 開始寫作
- 閱讀 [部署](./deployment.md) 將站點發佈上線
