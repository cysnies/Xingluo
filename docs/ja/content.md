...
- 投稿 URL はファイルパスから導出されます（[アーキテクチャ概要](./architecture.md) のルーティングセクションを参照）
...
heroImage: "./hero.png" # オプション、ヒーロー画像（戻るボタンとタイトルの間に表示、カード右側にも表示）
heroImageFit: "cover" # オプション、ヒーロー画像の適合モード（cover トリミング / contain 全体表示）、デフォルト cover
...
| `modDatetime`    | date                     | —               | 更新時間；公開日時と同時に表示                                                                                                                                         |
...
| `heroImage`      | image \| string          | —               | ヒーロー画像、詳細ページの戻るボタンとタイトルの間に表示、カード右側にも表示（`features.showPostCardHero`/`showPostDetailHero` で制御）                                |
| `heroImageFit`   | `"cover"` \| `"contain"` | `"cover"`       | ヒーロー画像の適合モード：`"cover"` はトリミングして填充（アスペクト比維持、端が切れる可能性あり）；`"contain"` は全体表示（アスペクト比維持、余白ができる可能性あり） |
| `canonicalURL`   | string                   | —               | 正規リンク、デフォルトを上書き（[SEO](./seo.md) 参照）                                                                                                                 |
...
ルーティング層が言語ごとに適切な翻訳を自動解決し、リストで重複排除します。翻訳がない投稿は元のコンテンツにフォールバックします。[国際化](./i18n.md) を参照。
...
---
title: "About"
description: "About this site" # オプション
ogImage: "default-og.jpg" # オプション、文字列のみ
canonicalURL: "https://..." # オプション
---
```
...
詳細は[メディアプレーヤー](./media-players.md)を参照。
...
コメントシステムは投稿詳細ページの下部に自動的にレンダリングされます（`features.comments` でプロバイダを設定）。[コメントシステム](./comments.md) を参照。
