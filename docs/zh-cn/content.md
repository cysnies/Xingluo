...
heroImageFit: "cover" # 可选，文章头图适配方式（cover 裁切填充 / contain 完整缩放），默认 cover
...
| `heroImageFit`   | `"cover"` \| `"contain"` | `"cover"`       | 文章头图适配方式：`"cover"` 裁切填充（保持宽高比，可能裁切边缘）；`"contain"` 完整缩放（保持宽高比，可能留白）                                                                  |
