...
site: {
  ...
  description: "Astro와 shadcn으로 구축된 현대적인 블로그 CMS",
...
| `mdx`              | `true`             | MDX 파싱 및 렌더링 활성화([콘텐츠 작성](./content.md) 참조)                |
| `comments`         | `{provider:false}` | 댓글 시스템 설정([댓글 시스템](./comments.md) 참조)                        |
| `players.aplayer`  | `false`            | APlayer 오디오 플레이어 활성화([미디어 플레이어](./media-players.md) 참조) |
