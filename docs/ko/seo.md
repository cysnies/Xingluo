...
[`src/layouts/Layout.astro`](../src/layouts/Layout.astro)의 `<head>`는 다음을 출력합니다:
...
- `theme-color` (런타임에 `theme.ts`에 의해 채워짐)
- `google-site-verification` (조건부)

## 게시물 페이지 메타

[`src/layouts/PostLayout.astro`](../src/layouts/PostLayout.astro)는 `<Fragment slot="head">`를 통해 게시물별 메타를 주입합니다:
...
게시물 상세 페이지(`PostDetailView.astro`)의 표준 URL 전략:

1. Frontmatter의 사용자 정의 `canonicalURL` → 우선 사용
2. 현재 게시물이 이 언어의 **실제 번역**(`locale`이 페이지 언어와 일치) → 자체 URL을 가리킴
3. 현재 게시물이 **대체 콘텐츠**(번역 없음, 원본 사용) → 기본 언어 원본 URL을 가리킴

전략 3은 검색 엔진이 독립적인 번역이 없는 페이지를 중복 콘텐츠로 처리하지 않도록 합니다. 독립적인 번역이 있는 게시물은 표준 URL이 자신을 가리키며 별도로 색인될 수 있습니다.

## BreadcrumbList 구조화 데이터

빵 부스러기가 있는 모든 페이지(게시물 목록, 태그 색인, 태그 게시물 목록, 아카이브, 정보, 검색)는 자동으로 `BreadcrumbList` JSON-LD 구조화 데이터를 출력합니다:
...
마지막 빵 부스러기 항목에 링크가 없는 경우(현재 페이지), 현재 페이지 URL이 `item`으로 사용됩니다. 게시물 상세 페이지는 빵 부스러기 컴포넌트를 사용하지 않으므로 이 구조화 데이터를 출력하지 않습니다.
...
OG 이미지는 Noto Sans SC를 사용합니다(`astro.config.ts`의 `fonts` 설정, CSS 변수 `--font-og` 참조). `astro:assets`의 `fontData`를 통해 로드됩니다. 이 글꼴은 satori 전용이며 사이트 CSS에 주입되지 않습니다.

### 폴백

- 글꼴을 사용할 수 없는 경우(네트워크 없음) → 1×1 플레이스홀더 PNG로 폴백(빌드 실패하지 않음)
- `dynamicOgImage` 비활성화 → `public/`의 정적 기본 OG 이미지 사용

### 게시물 OG 이미지 해상도

`PostDetailView.astro`의 4단계 폴백:

1. frontmatter `ogImage`가 문자열 → 직접 사용
2. frontmatter `ogImage`가 `image()` 객체 → `.src` 사용
3. `dynamicOgImage` 활성화 → 게시물 수준 `og.png` 엔드포인트 사용
...
- 제목, 설명, 사이트 URL은 `site` 설정에서 가져옴
- 항목은 `getSortedPosts`에서 가져옴(초안과 예약된 게시물은 이미 필터링됨)
...
- `i18n`: 자동 hreflang 생성 활성화, `zh-cn → zh-CN`, `en → en` 매핑, defaultLocale은 `zh-cn`

`sitemap-index.xml` 및 언어별 대체 선언을 생성합니다. `robots.txt`가 사이트맵을 참조합니다.

## hreflang 다국어 선언

`Layout.astro`는 각 언어에 대해 `<link rel="alternate">`를 출력합니다:
...
경로는 접두사 제거 후 `parseLocaleFromPath(stripBase(...))`을 통해 정규화되어 각 언어가 올바른 URL에 매핑됩니다. `x-default`는 기본 언어를 가리킵니다.
...
Google Search Console 인증은 `site.googleVerification`을 통해 두 가지 방식으로 구성됩니다:
