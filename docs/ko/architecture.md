...
```
xingluo/
├── astro.config.ts          # Astro 설정(통합, i18n, markdown, 글꼴, 환경)
├── xingluo.config.ts        # 사용자 설정 진입점
├── tsconfig.json            # TypeScript 설정(strict + @/* 경로 별칭)
├── package.json             # 종속성 및 스크립트
├── public/                  # 정적 자산(favicon.svg, 기본 OG 이미지 등)
├── docs/                    # 프로젝트 문서(이 디렉토리)
├── references/              # 읽기 전용 참조 프로젝트 소스(의존 금지)
└── src/
    ├── config.ts            # 기본 설정 병합, 해결된 설정 내보내기
    ├── content.config.ts    # 콘텐츠 컬렉션 스키마(posts, pages)
    ├── env.d.ts             # 서드파티 모듈 및 환경 변수 타입 선언
    ├── assets/              # 아이콘 컴포넌트
...
`src/utils/`에 플러그인 파일을 생성하고, `astro.config.ts`의 `markdown.remarkPlugins` 또는 `rehypePlugins`에 필요에 따라 주입합니다.
