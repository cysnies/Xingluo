...
| `global.css`     | Tailwind 진입점, 베이스 레이어, 커스텀 유틸리티, 콜아웃 테마 |
| `typography.css` | `.app-prose` 타이포그래피 및 코드 블록 스타일                |

## 테마 변수

`theme.css`는 OKLCH 색상 공간을 사용하여 시맨틱 변수를 정의하며, 라이트 및 다크 세트로 구성됩니다:
...
- `getPreferredTheme`: localStorage 우선, 시스템 기본 설정으로 폴백
...
- View Transitions 적응: `astro:after-swap`에서 재바인딩, `astro:before-swap`에서 theme-color 전달
- 시스템 `prefers-color-scheme` 변경 수신(사용자가 명시적으로 선택하지 않은 경우에만 따름)

### 댓글 및 플레이어 테마 동기화

- giscus: `postMessage({giscus:{setConfig:{theme}}})`를 통해 전환
- waline: `dark:"html.dark"` 선택자가 자동으로 따라감
- twikoo: `.dark` 클래스 변경을 감시하고 다시 빌드함(twikoo는 런타임 전환 미지원)
- [댓글 시스템](./comments.md) 참조
...
| X (소셜)  | `fa-brands:twitter` (FA5에는 x-twitter 없음) |

### 소셜 아이콘 동적 확인

[`src/lib/socialIcons.ts`](../src/lib/socialIcons.ts)는 `import.meta.glob`을 통해 파일 이름별로 `src/assets/icons/socials/*.astro`를 수집합니다. `getSocialIcon(name)`이 이름으로 확인합니다. 소셜 플랫폼 추가는 `socials/` 아래에 아이콘 파일을 추가하기만 하면 됩니다.

## 테마 사용자 정의

`src/styles/theme.css`의 CSS 변수를 편집하여 사이트 전체 색상을 조정합니다. 예를 들어 파란색 기본으로 전환하려면:
...
`bg-primary` / `text-primary`를 참조하는 모든 컴포넌트가 자동으로 따라옵니다.
