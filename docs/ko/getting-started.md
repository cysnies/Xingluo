...
2. **`astro build`**: 전체 사이트를 `dist/`에 정적으로 생성합니다(동적 OG 이미지, RSS, 사이트맵, robots.txt, Flexsearch 검색 인덱스 포함)
3. **`node scripts/generateSearchIndex.mjs`**: `dist/`를 스캔하여 `dist/search/`에 Flexsearch 검색 인덱스를 생성합니다

> 참고: 검색 인덱스 생성 스크립트는 Flexsearch 기반이며, 빌드 시 자동으로 실행됩니다.
