...
Configuração `i18n` do Astro (veja `astro.config.ts`):
...
**Importante: `prefixDefaultLocale: false` não gera automaticamente cópias de página localizadas** — você deve manter as rotas espelho `[locale]/` manualmente.

Abordagem do Xingluo:

- **Páginas raiz** = idioma padrão (`zh-cn`), sem prefixo de URL, ex. `/posts/welcome/`
- **`src/pages/[locale]/`** espelha todas as páginas; `getStaticPaths` usa `getLocaleParams()` para gerar apenas locais não padrão, ex. `/en/posts/welcome/`
- Páginas espelho também são wrappers finos, reutilizando o mesmo componente View para a lógica de renderização
...
Os componentes View usam `Astro.currentLocale` para resolução automática:

- Páginas raiz → `zh-cn`
- Páginas com segmento `[locale]` → `en` (ou outros locais não padrão)

Nenhuma verificação de caminho é necessária na camada de componente; `useTranslations(locale)` obtém as strings correspondentes diretamente.
...
- `post`: postagem (data, compartilhar, tags, voltar, editar, TOC, copiar código, lightbox de imagem, etc.)
- `pagination`: paginação
- `home`: página inicial (links sociais, em destaque, recentes)
...
- Posts com localidade não padrão têm canonical apontando para o original da localidade padrão (para evitar penalidades de conteúdo duplicado; veja [SEO](./seo.md))
...
4. **`src/pages/[locale]/`**: páginas espelho geram automaticamente a versão `ja` (`getLocaleParams` cobre isso)
...
1. **Post no idioma padrão**: coloque em `src/content/posts/<slug>.md`, defina `translationKey` como identificador do grupo:
...
- Nomes de subdiretórios de idioma devem corresponder aos códigos de idioma em `i18n.locales` do `astro.config.ts`
- Subdiretórios de idioma são filtrados do slug da URL (ex. `/posts/welcome/`, não `/posts/en/welcome/`)
...
| -------------------------------------------- | ------------------------------------------------------------------------------- |
| Acesso à localidade padrão a um post `zh-cn` | Renderiza o original no idioma padrão                                           |
| Localidade não padrão com **tradução**       | Renderiza a tradução correspondente                                             |
| Localidade não padrão **sem** tradução       | Recorre ao original no idioma padrão (conteúdo idêntico, canonical protege SEO) |

### Desduplicação de Lista

Páginas de lista (início, lista de posts, tags, arquivos, RSS) usam `getPostsForLocale` para selecionar posts representativos por idioma: cada grupo de tradução mostra apenas um card no idioma de destino, evitando entradas duplicadas para o mesmo tópico.

### canonical & SEO

- **Tem uma tradução independente**: canonical aponta para a própria URL da tradução, indexável separadamente pelos mecanismos de busca
- **Sem tradução (fallback)**: canonical aponta para o original no idioma padrão, evitando penalidades de conteúdo duplicado
- Declarações hreflang cobrem todos os idiomas, informando aos mecanismos de busca sobre as relações entre as versões de idioma
