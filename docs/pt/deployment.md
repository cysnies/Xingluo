...
- Ativos JS / CSS / fontes sob `_astro/`
- O índice de busca `search/`
- A `og.png` do site e por post
- `rss.xml`, `sitemap-index.xml`, `robots.txt`
- Ativos estáticos sob `public/` (favicon, imagem OG padrão, etc.)

## Variáveis de Ambiente

Definidas no momento da build:
...
Implante via GitHub Actions; fluxo de trabalho de exemplo:
...
> Se implantar sob um subcaminho (ex. `https://user.github.io/repo/`), defina `base: "/repo/"` em `astro.config.ts`.

### Nginx / Auto-hospedado

Faça upload de `dist/` para o servidor; configuração Nginx de exemplo:
...
- Ativos sob `_astro/` têm nomes de arquivo hash e podem ser armazenados em cache a longo prazo (`immutable`)
- Arquivos HTML não devem ser armazenados em cache (ou apenas brevemente) para garantir atualizações oportunas de conteúdo
- Índices Flexsearch carregam sob demanda; nenhuma estratégia especial de cache necessária
...
| ---------------------- | ------------------------------------------------------------------------------------------------------------------- |
| giscus                 | Nenhum; use o serviço público giscus.app (ou auto-hospede [giscus-vercel](https://github.com/giscus/giscus-vercel)) |
| twikoo                 | Implante o servidor twikoo (Vercel / CloudBase / auto-hospedado)                                                    |
| waline                 | Implante o servidor waline (Vercel / Cloudflare / auto-hospedado)                                                   |

Consulte a documentação oficial de cada sistema de comentários e [Sistema de comentários](./comments.md).
