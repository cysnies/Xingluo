...
- Beitrags-URLs werden aus dem Dateipfad abgeleitet (siehe Routing-Abschnitt in [Architekturübersicht](./architecture.md))
...
Details finden Sie unter [Medienplayer](./media-players.md).
...
Das Kommentarsystem wird automatisch am unteren Rand der Beitragsdetailseiten angezeigt (konfigurieren Sie den Anbieter in `features.comments`). Siehe [Kommentarsystem](./comments.md).
