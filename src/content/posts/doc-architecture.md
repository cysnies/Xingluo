...
`pnpm run build` = `astro check && astro build && node scripts/generateSearchIndex.mjs && node scripts/cleanOrphanAssets.mjs`
...
3. **`node scripts/generateSearchIndex.mjs`**：扫描 `dist/` 的 HTML，按语言生成搜索索引数据到 `dist/search/`
