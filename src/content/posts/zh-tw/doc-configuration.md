...
description: "星羅全部可設定項詳解，包括站點設定、文章設定、功能開關、社交連結、分享連結與環境變數。"
tags:
  - documentation
  - configuration
...
| `mdx`              | `true`             | 是否啟用 MDX 解析與渲染（詳見 [內容創作](./doc-content.md)）         |
| `comments`         | `{provider:false}` | 評論系統設定（詳見 [評論系統](./doc-comments.md)）                   |
| `players.aplayer`  | `false`            | 啟用 APlayer 音樂播放器（詳見 [媒體播放器](./doc-media-players.md)） |
