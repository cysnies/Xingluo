...
description: "星羅內容創作指南，涵蓋文章 frontmatter、Markdown/MDX 語法、程式碼凸顯、標註框與內容增強。"
tags:
  - documentation
  - writing
...
- 文章 URL 由檔案路徑推導（詳見 [架構總覽](./doc-architecture.md) 的路由體系）
...
| `modDatetime`    | date            | —               | 更新時間，與發佈時間同時展示，格式如"發佈於 2026-06-19 · 更新於 2026-06-20" |
...
| `canonicalURL`   | string          | —               | 規範連結，覆蓋預設（詳見 [SEO](./doc-seo.md)）                              |
...
路由層會自動解析對應語言的譯文並在列表中去重——不同語言的同一篇文章只渲染為對應語言的一張卡片。無譯文的文章在非預設語言頁會回退顯示原文內容。詳見 [國際化](./doc-i18n.md)。
...
文章詳情頁底部自動渲染評論系統（需在 `features.comments` 設定 provider）。詳見 [評論系統](./doc-comments.md)。
