...
description: "Guía del sistema de comentarios de Xingluo que cubre la selección, configuración e integración de giscus, twikoo y waline."
tags:
  - documentation
  - comments
...
features: {
  comments: {
    ...
    giscus: { /* configuración de giscus */ },
    // twikoo: { /* configuración de twikoo */ },
    // waline: { /* configuración de waline */ },
  },
}
```
...
comments: {
  ...
  giscus: {
    ...
    mapping: "pathname",          // opcional, mapeo página-discusión
    strict: false,                // opcional, coincidencia estricta de título
    reactionsEnabled: true,       // opcional, reacciones
    inputPosition: "bottom",      // opcional, posición de la caja de comentarios: top | bottom
...
comments: {
  provider: "twikoo",
  twikoo: {
    envId: "https://your-twikoo.example.com", // ID de entorno cloud o URL completa de autoalojamiento
...
comments: {
  ...
  waline: {
    ...
    dark: "html.dark",                       // opcional, selector oscuro (por defecto .dark del sitio)
  },
}
```
...
Consulta la [documentación de Waline](https://waline.js.org/) para implementar el servidor (Vercel / Cloudflare / autoalojado funcionan todos), luego coloca la dirección en `serverURL`.
...
Consulta [`src/scripts/comments.ts`](../src/scripts/comments.ts).
...
Cambia `features.comments.provider` en `xingluo.config.ts`; no se necesitan cambios de código. Xingluo renderiza el subcomponente correspondiente automáticamente.

### Añadir un sistema de comentarios

1. Crea un nuevo componente en `src/components/comments/` (ej. `Disqus.astro`) que renderice un placeholder de montaje
2. Añade una nueva rama de proveedor en el renderizado condicional de `Comments.astro`
3. Añade la lógica de inicialización en `src/scripts/comments.ts`
4. Extiende `CommentProvider` y los tipos de configuración en `src/types/config.ts`
