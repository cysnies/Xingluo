...
description: "Guia do sistema de comentários do Xingluo cobrindo seleção, configuração e integração de giscus, twikoo e waline."
tags:
  - documentation
  - comments
...
A seção de comentários aparece apenas na parte inferior das **páginas de detalhe do post** (após a navegação anterior/próximo), renderizada por [`src/components/comments/Comments.astro`](../src/components/comments/Comments.astro).
...
comments: {
  ...
  giscus: {
    ...
    mapping: "pathname",          // opcional, mapeamento página-para-discussão
...
comments: {
  provider: "twikoo",
  twikoo: {
    envId: "https://your-twikoo.example.com", // ID do ambiente cloud ou URL de auto-hospedagem completa
...
comments: {
  ...
  waline: {
    ...
    dark: "html.dark",                       // opcional, seletor escuro (padrão site .dark)
  },
}
```
...
Consulte a [documentação do Waline](https://waline.js.org/) para implantar o servidor (Vercel / Cloudflare / auto-hospedagem), então coloque o endereço em `serverURL`.
...
Veja [`src/scripts/comments.ts`](../src/scripts/comments.ts).
...
| twikoo                 | Observa mudanças de classe `.dark` e reconstrói a instância |
