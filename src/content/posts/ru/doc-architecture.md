...
description: "Обзор архитектуры Xingluo, охватывающий структуру каталогов, поток конфигурации, поток рендеринга, конвейер сборки и руководство по расширению."
tags:
  - documentation
  - architecture
...
└── src/
    ├── config.ts            # Объединение значений по умолчанию, экспорт разрешённой конфигурации
    ├── content.config.ts    # Схемы коллекций контента (posts, pages)
    ├── env.d.ts             # Объявления типов для сторонних модулей и переменных окружения
    ├── assets/              # Компоненты иконок
    │   └── icons/           # astro-icon + Font Awesome (включает socials/)
    ├── components/          # UI-компоненты
    │   ├── ui/              # shadcn-стиль компоненты (Button, Card, Badge и др.)
    │   ├── post/            # Компоненты страницы записи (нав пред/след, назад, поделиться и др.)
    ...
    │   ├── postEnhancements.ts # Улучшения записи (якоря, копирование, лайтбокс, прогресс)
    ...
    └── utils/               # Вспомогательные функции
        ├── getPostPaths.ts  # Вывод слага и URL записи
        ├── getSortedPosts.ts# Сортировка записей
        ├── postFilter.ts    # Фильтрация черновиков и запланированных записей
...
### Добавление плагина remark/rehype

Создайте файл плагина в `src/utils/` и инжектируйте его по мере необходимости в `markdown.remarkPlugins` или `rehypePlugins` в `astro.config.ts`.
```
