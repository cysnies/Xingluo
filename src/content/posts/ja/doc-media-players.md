...
description: "Xingluo メディアプレーヤーガイド。Markdown フェンスと MDX コンポーネントを使用した APlayer オーディオと DPlayer ビデオの使用方法をカバーします。"
tags:
  - documentation
  - media-players
...
APlayerとDPlayerには公式のTypeScript型がありません。Xingluoは [`src/types/aplayer.d.ts`](../src/types/aplayer.d.ts) と [`src/types/dplayer.d.ts`](../src/types/dplayer.d.ts) に緩やかなモジュール宣言を提供し、スプレッド互換性のためにオプションフィールドはオプショナルに設定されています。MDXコンポーネントのPropsは完全な型制約を持ちます。
