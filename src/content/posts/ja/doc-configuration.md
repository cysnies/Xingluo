...
description: "Xingluo のすべての設定オプションの完全リファレンス。サイト設定、投稿設定、機能トグル、ソーシャルリンク、共有リンク、環境変数を含みます。"
tags:
  - documentation
  - configuration
...
| `mdx`              | `true`             | MDX の解析とレンダリングを有効化（[コンテンツ作成](./doc-content.md) を参照）               |
| `comments`         | `{provider:false}` | コメントシステムの設定（[コメントシステム](./doc-comments.md) を参照）                      |
| `players.aplayer`  | `false`            | APlayer オーディオプレーヤーを有効化（[メディアプレーヤー](./doc-media-players.md) を参照） |
